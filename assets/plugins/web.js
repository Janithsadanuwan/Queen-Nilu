...
Nilu.operate({pattern: 'speedtest ?(.*)', fromMe: true, desc: Lang.SPEEDTEST_DESC}, (async (message, match) => {
    await QueenNilu.nilu_setup()
    if (match[1] == 'user' || match[1] == 'User') {
        
        // Preliminary Message
        var transition_message = await QueenNilu.speedtest_begin(Build.LANG)
        ...
        // Speedtest Modules
        var user_download = await QueenNilu.speedtest_user()
        var user_upload = await QueenNilu.speedtest_userup()
        var auth_message = await QueenNilu.speedtest_message(Build.LANG)
        ...
        // Speedtest Modules
        var server_download = await QueenNilu.speedtest_server()
        var server_upload = await QueenNilu.speedtest_serverup()
        var auth_message = await QueenNilu.speedtest_message(Build.LANG)
        var act_ping = end - start
        ...
        // Preliminary Message
        var transition_message = await QueenNilu.speedtest_once(Build.LANG)
...
Nilu.operate({pattern: 'ping', fromMe: true, deleteCommand: false, desc: Lang.PING_DESC, dontAddCommandList: true}, (async (message, match) => {
  var start = new Date().getTime();
...
Nilu.operate({pattern: 'short ?(.*)', fromMe: false, desc: Lang.URL}, (async (message, match) => {
...
Nilu.operate({pattern: 'getqr ?(.*)', fromMe: LOL, desc: Lang.NILU_QR,  deleteCommand: false}, (async (message, match) => {
