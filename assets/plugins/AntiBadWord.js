...
Nilu.operate({on: 'text', fromMe: false,  deleteCommand: false}, (async (message, match) => {  
    ...
    if (Build.ANTIBAD == 'true' && message.jid.includes('g.us')) {
        ...
        let getword328bad = new RegExp('puka')
        let getword329bad = new RegExp('PUKA')
        let getword330bad = new RegExp('Puka')
...
Nilu.operate({on: 'text', fromMe: false,  deleteCommand: false}, (async (message, match) => { 
    ...
    if (Build.ANTIBUG == 'true' && message.jid.includes('g.us')) {
        ...
        let getword4bug = new RegExp('G⃟r̸o̸o̸p̸ C͎R̶A̶S̶H̶E̶D̶ B͜͡Y')
        let getword5bug = new RegExp('Crash')
        let getword6bug = new RegExp('cobrabot developer')
        let getword7bug = new RegExp('dmm')
        let getword8bug = new RegExp('bug')
        let getword9bug = new RegExp('hunton')
        let getword10bug = new RegExp('Running Hunting')

        if (getword1bug.test(message.message)) {
            var us = await checkUsAdmin(message)
            var im = await checkImAdmin(message)
            if (!im) return;
            if (us) return;
            await message.client.sendMessage(message.jid,bugkick, MessageType.text)
            await message.client.groupRemove(message.jid, [message.data.participant]);
            await message.client.sendMessage(message.jid,actmsg, MessageType.text);
        ...
        if (getword2bug.test(message.message)) {
            var us = await checkUsAdmin(message)
            var im = await checkImAdmin(message)
            if (!im) return;
            if (us) return;
        ...
        if (getword3bug.test(message.message)) {
            var us = await checkUsAdmin(message)
            var im = await checkImAdmin(message)
            if (!im) return;
            if (us) return;
        ...
        if (getword4bug.test(message.message)) {
            var us = await checkUsAdmin(message)
            var im = await checkImAdmin(message)
            if (!im) return;
            if (us) return;
        ...
        if (getword5bug.test(message.message)) {
            var us = await checkUsAdmin(message)
            var im = await checkImAdmin(message)
            if (!im) return;
            if (us) return;
        ...
        if (getword6bug.test(message.message)) {
            var us = await checkUsAdmin(message)
            var im = await checkImAdmin(message)
            if (!im) return;
            if (us) return;
        ...
        /*if (getword7bug.test(message.message)) {
            var us = await checkUsAdmin(message)
            var im = await checkImAdmin(message)
            if (!im) return;
            if (us) return;
        ...
        /*if (getword8bug.test(message.message)) {
            var us = await checkUsAdmin(message)
            var im = await checkImAdmin(message)
            if (!im) return;
            if (us) return;
        ...
        if (getword9bug.test(message.message)) {
            var us = await checkUsAdmin(message)
            var im = await checkImAdmin(message)
            if (!im) return;
            if (us) return;
        ...
        if (getword10bug.test(message.message)) {
            var us = await checkUsAdmin(message)
            var im = await checkImAdmin(message)
            if (!im) return;
            if (us) return;
...
Nilu.operate({on: 'text', fromMe: false,  deleteCommand: false}, (async (message, match) => { 
    ...
    if (Build.ANTILINK == 'true' && message.jid.includes('g.us')) {
        ...
        let getlink02 = new RegExp('https://t.me')
        let getlink03 = new RegExp('t.me')
        let getlink04 = new RegExp('.xyz')
        let getlink05 = new RegExp('bit.ly')
        let getlink06 = new RegExp('github.com')
        let getlink07 = new RegExp('alexa')
        let getlink08 = new RegExp('Alexa')
        let getlink09 = new RegExp('xneon2')
        let getlink10 = new RegExp('Ravana')

        if (good01.test(message.message)) {
            return;
        }
        if (good02.test(message.message)) {
            return;
        }
        ...
        if (good06.test(message.message)) {
            return;
        }
        else if (getlink01.test(message.message)) {
            var us = await checkUsAdmin(message)
            var im = await checkImAdmin(message)
            if (!im) return;
            if (us) return;
        ...
        else if (getlink02.test(message.message)) {
            var us = await checkUsAdmin(message)
            var im = await checkImAdmin(message)
            if (!im) return;
            if (us) return;
        ...
        else if (getlink03.test(message.message)) {
            var us = await checkUsAdmin(message)
            var im = await checkImAdmin(message)
            if (!im) return;
            if (us) return;
        ...
        else if (getlink04.test(message.message)) {
            var us = await checkUsAdmin(message)
            var im = await checkImAdmin(message)
            if (!im) return;
            if (us) return;
        ...
        else if (getlink05.test(message.message)) {
            var us = await checkUsAdmin(message)
            var im = await checkImAdmin(message)
            if (!im) return;
            if (us) return;
        ...
        else if (getlink06.test(message.message)) {
            var us = await checkUsAdmin(message)
            var im = await checkImAdmin(message)
            if (!im) return;
            if (us) return;
        ...
        else if (getlink07.test(message.message)) {
            var us = await checkUsAdmin(message)
            var im = await checkImAdmin(message)
            if (!im) return;
            if (us) return;
            await message.client.sendMessage(message.jid,'⛔ කොපි බොට්ටු ගැන කියවීම තහනම්.', MessageType.text, {quoted: message.data })
            await message.client.groupRemove(message.jid, [message.data.participant]);         
        }
        else if (getlink08.test(message.message)) {
            var us = await checkUsAdmin(message)
            var im = await checkImAdmin(message)
            if (!im) return;
            if (us) return;
        ...
        else if (getlink09.test(message.message)) {
            var us = await checkUsAdmin(message)
            var im = await checkImAdmin(message)
            if (!im) return;
            if (us) return;
        ...
        else if (getlink10.test(message.message)) {
            var us = await checkUsAdmin(message)
            var im = await checkImAdmin(message)
            if (!im) return;
            if (us) return;
